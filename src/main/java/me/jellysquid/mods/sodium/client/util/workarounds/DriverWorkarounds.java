...
import me.jellysquid.mods.sodium.client.util.workarounds.platform.nvidia.NvidiaWorkarounds;
...
public class DriverWorkarounds {
    ...
    private static void apply(Set<Reference> active) {
        if (active.contains(Reference.ISSUE_1486)) {
            NvidiaWorkarounds.forceInstallProfile();
        }
    }

    private static Set<Reference> scan(DriverProbeResult probe) {
